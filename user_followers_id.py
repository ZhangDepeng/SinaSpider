...
if __name__ == "__main__":
    ...
    with open('F:/users19_id.csv', 'wb') as f:
            f.write('\xEF\xBB\xBF')
            writer = csv.writer(f)
            writer.writerow(['粉丝ID', 'poiid'])
            poi_id_file = open("poiID1.txt", "r")
            for lines in poi_id_file:
                print lines
                try:
                    # print client.friendships.followers.ids()
                    # followers = client.friendships.followers.ids()['followers']
                    pnt = client.friendships.followers.get(poiid=lines, count=5000)
                    print pnt
                    count = lines
                    for user in pnt.followers:
                        writer.writerow(
                            [user.ids.decode('utf-8'), count])
                        print user.id.decode('utf-8'), \
                         count
                except:
                    print "err"
    f.close()