...
class SinaAPI():
    ...
    def __init__(self, CALLBACK_URL, APP_KEY, REDIRECT_URL, USER_ID, USER_PSWD):
        ...
        self.APP_KEY ='2324300722'
        self.REDIRECT_URL ='https://api.weibo.com/oauth2/default.html'
        self.USER_ID = '17862513091'
        self.USER_PSWD ='lsh1996'
...
if __name__ == "__main__":
    APP_KEY = '2324300722'  # app key
    APP_SECRET = '2155cf934da23cccc5d5af7ba158458e'  # app secret
    ...
    with open('F:/usershow_id.csv', 'wb') as f:
            f.write('\xEF\xBB\xBF')
            writer = csv.writer(f)
            writer.writerow(['地点', '性别', '粉丝数', '关注数', 'poiid'])
            poi_id_file = open("poiID1.txt", "r")
            for lines in poi_id_file:
                print lines
                try:
                    # print client.friendships.friends()
                    # friends = client.friendships.friends()['friends']
                    pnt = client.friendships.friends(poiid=lines, count=200, uid=64)
                    print pnt
                    count = lines
                    for user in pnt.friends:
                        writer.writerow(
                            [user.location.decode('utf-8'), user.gender.decode('utf-8'), user.followers_count.decode('utf-8'), user.friends_count.decode('utf-8'), count])
                        print user.location.decode('utf-8'), user.gender.decode('utf-8'), user.followers_count.decode('utf-8'), user.friends_count.decode('utf-8'), \
                          count
                except:
                    print "err"
    f.close()