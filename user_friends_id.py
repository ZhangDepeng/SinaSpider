...
class SinaAPI():
    ...
    def __init__(self, CALLBACK_URL, APP_KEY, REDIRECT_URL, USER_ID, USER_PSWD):
        ...
        self.USER_ID = '1435529236@qq.com'
        self.USER_PSWD ='flyover2015'
...
if __name__ == "__main__":
    ...
    with open('F:/userFriends1.csv', 'wb') as f:
            f.write('\xEF\xBB\xBF')
            writer = csv.writer(f)
            writer.writerow(['关注ID', 'userID'])
            poi_id_file = open("poiID1.txt", "r")
            for lines in poi_id_file:
                print lines
                try:
                    # print client.friendships.friends.ids()
                    # friends = client.friendships.friends.ids()['friends']
                    pnt = client.friendships.friends.ids.get(uid=lines, count=5000)
                    print pnt
                    count = lines
                    for user in pnt.ids:
                        writer.writerow(
                        [user.ids.decode('utf-8'), count])
                        print user.id.decode('utf-8'),count
                except:
                    print "err"
    f.close()