...
class SinaAPI():
    ...
    def __init__(self, CALLBACK_URL, APP_KEY, REDIRECT_URL, USER_ID, USER_PSWD):
        ...
        self.APP_KEY ='2745707827'
        self.REDIRECT_URL ='https://api.weibo.com/oauth2/default.html'
        self.USER_ID = '17865818216'
        self.USER_PSWD ='zyd9708.qxqzx'
...
if __name__ == "__main__":
    APP_KEY = '2745707827'  # app key
    APP_SECRET = '22352c3c5741b3aaf8cb5bab99b62f78'  # app secret
    ...
    with open('I:/us4_id.csv', 'wb') as f:
            f.write('\xEF\xBB\xBF')
            writer = csv.writer(f)
            writer.writerow(['用户ID', '地点', '创建时间', '用户性别', 'poiid','创建时间'])
            ...
            for lines in poi_id_file:
                ...
                try:
                    ...
                    for user in pnt.users:
                        writer.writerow(
                            [user.id.decode('utf-8'), user.location.decode('utf-8'), user.checkin_at.decode('utf-8'),
                             user.gender.decode('utf-8'), count,user.created_at.decode('utf-8')])
                        print user.id.decode('utf-8'), user.location.decode('utf-8'), user.checkin_at.decode(
                            'utf-8'), user.gender.decode('utf-8') , count,user . created_at.decode('utf-8')
                except:
                    print "err"
    f.close()